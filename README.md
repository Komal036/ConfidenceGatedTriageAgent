...
- **Render Free Tier memory constraints dictate the architecture.** To run SentenceTransformer and CrossEncoder simultaneously within 512MB RAM without out-of-memory (OOM) crashes, the app aggressively manages memory. It relies on the PyTorch CPU-only wheel, pre-imports massive C++ libraries during FastAPI's lifespan hook to avoid threadpool deadlocks, and loads/unloads models sequentially per request. This means processing a ticket takes ~10-15 seconds (cold-loading models from disk) - a deliberate trade-off prioritizing stability over speed on free hardware.
